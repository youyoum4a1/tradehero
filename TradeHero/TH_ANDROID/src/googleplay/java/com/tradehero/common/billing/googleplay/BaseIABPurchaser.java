        ...
        IABExceptionType>
{
    ...
    private void checkNotPurchasing()
    {
        if (purchasing)
        {
            throw new IllegalStateException("BaseIABPurchaser is already purchasing so it cannot be launched again");
        }
    }
    ...
    @Override public void purchase(@NonNull IABPurchaseOrderType purchaseOrder)
    {
        checkNotPurchasing();
        purchasing = true;
    ...
    @Override public boolean handleActivityResult(int requestCode, int resultCode, Intent data)
    {
        ...
        else
        {
            ...
            {
                Timber.w("Purchase failed. Result code: %d. Response: %s", resultCode, IABConstants
                        .getStatusCodeDescription(responseCode));
