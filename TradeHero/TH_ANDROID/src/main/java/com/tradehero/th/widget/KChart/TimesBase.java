 ...
 * 
 */
public class TimesBase extends View {
    ...
    public static int COLOR_RED = 0xFFff2731;
    public static int COLOR_GREEN = 0xFF0cb772;
	...
	public TimesBase(Context context) {
		super(context);
		init();
	}

	public TimesBase(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public TimesBase(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		longitudeSpacing = (viewWidth - 2 - mLeftMargin) / (DEFAULT_LOGITUDE_NUM + 1);
		...
		LOWER_CHART_TOP = viewHeight - 1 - latitudeSpacing * (DEFAULT_LOWER_LATITUDE_NUM + 1) - DEFAULT_AXIS_TITLE_SIZE;
		mLowerChartHeight = viewHeight - 2 - LOWER_CHART_TOP - DEFAULT_AXIS_TITLE_SIZE;
	...
	private void drawBorders(Canvas canvas, int viewHeight, int viewWidth) {
		...
		canvas.drawLine(1, 1, 1, viewHeight - 1 - DEFAULT_AXIS_TITLE_SIZE, paint);
		canvas.drawLine(viewWidth - 1, viewHeight - 1 - DEFAULT_AXIS_TITLE_SIZE, viewWidth - 1, 1, paint);
		canvas.drawLine(viewWidth - 1, viewHeight - 1 - DEFAULT_AXIS_TITLE_SIZE, 1, viewHeight - 1 - DEFAULT_AXIS_TITLE_SIZE, paint);
	}
	...
	private void drawLongitudes(Canvas canvas, int viewHeight, float longitudeSpacing) {
		...
		for (int i = 0; i <= DEFAULT_LOGITUDE_NUM; i++) {
			canvas.drawLine(1 + mLeftMargin + longitudeSpacing * i, topTitleHeight + 2, 1 + mLeftMargin + longitudeSpacing * i,
					UPER_CHART_BOTTOM, paint);
			canvas.drawLine(1 + mLeftMargin + longitudeSpacing * i, LOWER_CHART_TOP, 1 + mLeftMargin + longitudeSpacing * i,
					viewHeight - 1 - DEFAULT_AXIS_TITLE_SIZE, paint);
		}
	}
	...
	private void drawLatitudes(Canvas canvas, int viewHeight, int viewWidth, float latitudeSpacing) {
		...
		for (int i = 1; i <= DEFAULT_UPER_LATITUDE_NUM; i++) {
			canvas.drawLine(1 + mLeftMargin, topTitleHeight + 1 + (int)latitudeSpacing * i,
                    viewWidth - 1, topTitleHeight + 1 + (int)latitudeSpacing * i, paint);
		}
		for (int i = 1; i <= DEFAULT_LOWER_LATITUDE_NUM; i++) {
			canvas.drawLine(1 + mLeftMargin, viewHeight - 1 - DEFAULT_AXIS_TITLE_SIZE - (int)latitudeSpacing * i,
                    viewWidth - 1, viewHeight - 1 - DEFAULT_AXIS_TITLE_SIZE - (int)latitudeSpacing * i, paint);
		}
