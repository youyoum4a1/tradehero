package com.tradehero.th.models.provider.macquarie;

import com.tradehero.th.R;
import com.tradehero.th.models.provider.ProviderSpecificResourcesDTO;

/**
 * Created by xavier on 2/19/14.
 */
public class PhilipsMacquarieProviderSpecificResourcesDTO extends ProviderSpecificResourcesDTO
{
    public PhilipsMacquarieProviderSpecificResourcesDTO()
    {
        super();
        //mainCompetitionFragmentTitleResId = R.string.competition_macquarie_warrant_hero_main_title;
        notJoinedBannerImageResId = R.drawable.lb_philip_macquarie_large_notjoined;
        joinedBannerImageResId = R.drawable.lb_philip_macquarie_large_joined;
        tradeNowBtnImageResId = R.drawable.btn_philip_macquarie_tradenow;
        //helpVideoListFragmentTitleResId = R.string.competition_macquarie_warrant_hero_help_video_title;
        helpVideoLinkBackgroundResId = R.drawable.btn_provider_22_philips_macquarie;
        helpVideoLinkTextColourResId = R.color.white;
        //timedHeaderLeaderboardTitleResId = R.string.competition_macquarie_warrant_hero_main_title;
        competitionPortfolioTitleResId = R.string.competition_philip_macquarie_warrant_hero_portfolio_title;
        //securityListFragmentTitleResId = R.string.competition_macquarie_warrant_hero_security_list_title;
    }
}
