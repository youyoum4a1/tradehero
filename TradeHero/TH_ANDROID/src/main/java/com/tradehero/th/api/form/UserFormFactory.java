...
public class UserFormFactory {
    ...
    private static UserFormDTO createEmptyForType(String type) {
        ...
        if (type.equals(SocialAuthenticationProvider.QQ_AUTH_TYPE)) {
    ...
    private static void populatePerType(UserFormDTO userFormDTO, String type, JSONObject json)
        ...
        } else if (type.equals(SocialAuthenticationProvider.QQ_AUTH_TYPE)) {
            ((QQUserFormDTO) userFormDTO).qq_access_token =
                    json.getString(QQAuthenticationProvider.KEY_ACCESS_TOKEN);

            ((QQUserFormDTO) userFormDTO).qq_openid =
                    json.getString(QQAuthenticationProvider.KEY_OPENID);
        }
    }
}
