...
public abstract class SocialFriendsFragment extends DashboardFragment
        implements RecyclerSocialFriendsAdapter.OnElementClickListener, View.OnClickListener {
    @InjectView(R.id.friends_root_view) ViewAnimator rootView;
    @InjectView(R.id.social_friends_list) RecyclerView friendsListView;
    @InjectView(R.id.search_social_friends) EditText searchEdit;
    @InjectView(R.id.tradeheroprogressbar_invite_friends) TradeHeroProgressBar progressBar;
    ...
    protected RecyclerSocialFriendsAdapter socialFriendsListAdapter;
    ...
    public void setInviteAllViewCountText(int count) {
        if (count > 0) {
            ...
            inviteAllView.setEnabled(true);
        }
        if (count <= 0) {
            inviteAllView.setText(getString(R.string.invite));
            inviteAllView.setEnabled(false);
        }
    }
    ...
    private void initView() {
        ...
        inviteAllView.setOnClickListener(this);
        followAllView.setOnClickListener(this);
    ...
    private void displayContentView(@Nullable UserFriendsDTOList value) {
        ...
        if (value == null || value.size() == 0) {
            rootView.setDisplayedChild(1); //empty View
        } else {
            bindData();
            rootView.setDisplayedChild(0); //Content View
        }
    }
    ...
    protected void bindNormalData() {
        socialFriendsListAdapter = new RecyclerSocialFriendsAdapter(getActivity(), listedSocialItems);
        socialFriendsListAdapter.setOnElementClickListener(this);
    ...
    private boolean hasListData() {
        return friendsListView.getAdapter() != null && friendsListView.getAdapter().getItemCount() > 0;
    }
        ...
        public void afterTextChanged(@Nullable Editable s) {
            if (socialFriendsListAdapter != null) {
                if (s != null && s.toString().trim().length() > 0) {
                    ...
                    socialFriendsListAdapter.filter(s.toString());
                } else {
                    socialFriendsListAdapter.setItems(listedSocialItems);
                }
            }
        }
    }
